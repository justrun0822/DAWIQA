...
def performance_fit(y_label, y_output):
    # 检查是否有 NaN 值
    if np.isnan(y_output).any() or np.isinf(y_output).any():
        print("警告：预测输出包含 NaN 或 Inf 值，无法进行性能评估")
        return 0, 0, 0, float('inf')  # 返回默认值
    
    y_output_logistic = fit_function(y_label, y_output)
    PLCC = stats.pearsonr(y_output_logistic, y_label)[0] # 皮尔逊线性相关系数
    SRCC = stats.spearmanr(y_output, y_label)[0]         # 斯皮尔曼等级相关系数
    KRCC = stats.kendalltau(y_output, y_label)[0]        # 肯德尔等级相关系数
    RMSE = np.sqrt(((y_output_logistic-y_label) ** 2).mean()) # 均方根误差
